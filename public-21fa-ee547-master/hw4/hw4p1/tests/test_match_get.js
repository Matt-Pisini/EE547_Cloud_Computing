...
describe('GET /match', function() {
  ...
  context('sort match', function () {
    it('active, prize_usd DESC', async function () {
      ...
      await Promise.map(vals, prize_usd => fix._match_create({ prize_usd }));
    ...
    it('at most 4 not-active', async function () {
      ...
      const ids = await Promise.map(vals, prize_usd => fix._match_create({ prize_usd }));
  ...
  context('mid', function () {
    ...
    context('prize_usd', function () {
      it('response contains prize_usd', async function () {
        const { mid } = await fix._match_create();
        return fix.test_succeed(DEFAULT_METHOD, DEFAULT_PATH(mid), {}, 200, ['prize_usd']);
      });
  
      it('total_prize is currency', async function () {
        const prize_usd = '5.43';
        const { mid } = await fix._match_create({ prize_usd });
        return fix.test_succeed(DEFAULT_METHOD, DEFAULT_PATH(mid), {}, 200, { prize_usd });
      });
    });
